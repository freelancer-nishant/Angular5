...
export class AssessmentUploadsComponentCeldt implements OnInit {
    ...
    ngOnInit() {
        this.assessmentUploadsComponent.selectedOption = 'CELDT';
    }
    submit() {
        let responseResult: ResponseResult;
        this.assessmentService.saveCELDT(this.assessmentUploadsComponent.filesAdded).subscribe((result: any) => responseResult = result,
            (error: any) => {
                this.assessmentUploadsComponent.UploadErrorMsgs.push({ severity: 'error', summary: 'error Message', detail: error.error.message });
            },
            () => {
                this.assessmentUploadsComponent.UploadErrorMsgs.push({ severity: 'success', summary: 'success Message', detail: "CELDT added successfully." });
            });
