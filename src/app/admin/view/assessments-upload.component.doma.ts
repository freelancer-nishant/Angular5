...
export class AssessmentUploadsComponentDoma implements OnInit {
    ...
    ngOnInit() {
        this.assessmentUploadsComponent.selectedOption = 'LGL - DOMA Algebra'
    }

    submit() {
        let responseResult: ResponseResult;
        this.assessmentService.saveDOMAAlgebra(this.assessmentUploadsComponent.filesAdded).subscribe((result: any) => responseResult = result,
            (error: any) => {
                this.assessmentUploadsComponent.UploadErrorMsgs.push({ severity: 'error', summary: 'error Message', detail: error.error.message });
            },
            () => {
                this.assessmentUploadsComponent.UploadErrorMsgs.push({ severity: 'success', summary: 'success Message', detail: "DOMA Algebra added successfully." });
            });
    }
}
